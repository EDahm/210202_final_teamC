			...
			<div>
				...
				<div>
					<label>업체명</label> <input name='t_vstng_cmpny'
						value='<c:out value="${store.t_vstng_cmpny }" />' readonly="readonly">
				</div>
				<div>
					<label>업체주소</label> <input name='s_address'
						value='<c:out value="${store.s_address }" />' readonly="readonly">
				</div>
				<div>
					<label>연락처</label> <input name='s_contact'
						value='<c:out value="${store.s_contact }" />' readonly="readonly">
				</div>
				<div>
					<label>운영시간</label> <input name='s_biz_h'
						value='<c:out value="${store.s_biz_h }" />' readonly="readonly">
				</div>
				<div>
					<label>수거진행상태</label> <input name='s_progress'
						value='<c:out value="${store.s_progress }" />' readonly="readonly">
				</div>
				...
				<form id='operForm' action="/store/modify" method="get">
					<input type='hidden' id='t_vstng_cmpny' name='t_vstng_cmpny'
						value='<c:out value="${store.t_vstng_cmpny }"/>'> <input
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("button[data-oper='list']").on("click", function(e) {
			...
			operForm.find("#t_vstng_cmpny").remove();
			operForm.attr("action", "/store/list")
			operForm.submit();
		});
	});
</script>
